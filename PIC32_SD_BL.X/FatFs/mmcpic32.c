...
#define	FCLK_SLOW()	SPI_setCLKFreq(400000)		/* Set slow clock (100k-400k) */
#define	FCLK_FAST()	SPI_setCLKFreq(24000000)		/* Set fast clock (depends on the CSD) */
...
#define xmit_spi(dat) 	SPI_send(dat)
#define rcvr_spi()		SPI_send(0xff)
#define rcvr_spi_m(p)	*(p) = SPI_send(0xff);
...
static BYTE wait_ready (void){
	...
	do
		res = rcvr_spi();
	while ((res != 0xFF) && (start++ < INIT_TIMEOUT));
